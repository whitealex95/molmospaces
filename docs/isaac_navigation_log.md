...
| 6 | 2026-05-21 | Depth: probe the annotator / `Camera` sensor APIs | ✅ solved | Replicator annotators (viewport RP and a dedicated RP) returned empty data. The `isaacsim.sensors.camera.Camera` sensor works **in GUI mode** and yields RGB **and** metric depth — but it must be created at the **stage root** (`/ego_cam`), not under `/World`: the mansion USD's `/World` carries a det-1 reflection transform (the handedness fix) and IsaacSim's `XFormPrim` pose math (`scipy Rotation.from_matrix`) rejects a non-positive determinant. Pump with `world.step(render=True)`. |
| 7 | 2026-05-21 | G1 MJCF→USD import | ✅ works | `MJCFCreateImportConfig` + `MJCFCreateAsset` kit commands → `g1_isaac/g1.usd` (+ a 28 MB `configuration/g1_base.usd` geometry layer). Reference `g1_base.usd` for a static, physics-free G1. |
